...
export default function BasicTableFeeHead({
  amountList,
  // onActive,
  onSendData,
}: IProps) {
      ...
      <div className="max-w-full overflow-x-auto">
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {amountList?.data?.map((order: any, index: any) => (
        ...
        <div className="p-8">
          <Pagination
            count={count}
            onChange={setCount}
            length={amountList?.data?.length ? amountList?.data?.length : 1}
          />
        </div>
      </div>
    </div>
